...
<Tip>
  Before calling `claim`, use [`get-vault-status`](/contract/overview) to confirm the vault is in `claimable` state and [`get-heir-info`](/contract/overview) to verify `has-claimed` is `false`. This avoids submitting transactions that will fail.
</Tip>
